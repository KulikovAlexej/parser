        ...
        try:
            ...
            if len(tds) == 6:
                return {
                    ...
                    'score': tds[3].string,
                    'over': tds[4].get('data-odd'),
                    'under': tds[5].get('data-odd'),
                    'over_date': tds[4].get('data-created'),
                    'under_date': tds[5].get('data-created'),
                }
            elif len(tds) == 4 or len(tds) == 0:
                return None
            else:
...
for item in match_items:
    ...
    output_dir = prepare_dir('betexplorer', match_date, 'upcoming_odds_uo')
    ...
    if not os.path.isfile(filename) or os.path.getsize(filename) < 2000:
        # cmd = "curl 'http://www.betexplorer.com/gres/ajax/matchodds.php?p=1&e=" + match_id + "&b=ou' -H 'Cookie: new_version=1; my_timezone=%2B1; infobox-new_promo=1; _gat_UA-191939-1=1; js_cookie=1; widget_timeStamp=1484878529; my_cookie_id=63568996; my_cookie_hash=448bc8d270143e796ccefc656626874d; widget_pageViewCount=2; _ga=GA1.2.1301231815.1477358241' -H 'Accept-Encoding: gzip, deflate, sdch' -H 'Accept-Language: en-US,en;q=0.8,ru;q=0.6,und;q=0.4' -H 'User-Agent: Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36' -H 'Accept: application/json, text/javascript, */*; q=0.01' -H 'Referer: " + source_url + "' -H 'X-Requested-With: XMLHttpRequest' -H 'Connection: keep-alive' --compressed > " + filename
        # os.system(cmd)
        buffer = BytesIO()
        c = pycurl.Curl()
        c.setopt(c.URL, 'http://www.betexplorer.com/gres/ajax/matchodds.php?p=1&e=' + match_id + '&b=ou')
        c.setopt(pycurl.ENCODING, 'gzip')
        c.setopt(c.HTTPHEADER, (
            'Cookie: new_version=1; my_timezone=%2B1; infobox-new_promo=1; _gat_UA-191939-1=1; js_cookie=1; widget_timeStamp=1484878529; my_cookie_id=63568996; my_cookie_hash=448bc8d270143e796ccefc656626874d; widget_pageViewCount=2; _ga=GA1.2.1301231815.1477358241',
            'Accept-Encoding: gzip, deflate, sdch',
            'Accept-Language: en-US,en;q=0.8,ru;q=0.6,und;q=0.4',
        ...
        content = file.write(body.decode('iso-8859-1'))
        file.close()
    ...
    try:
        ...
        odds_uo = list(filter(lambda o: o is not None, map(extract_odds, trs)))
        ...
        connector.test.update({'key': match_id}, entity, upsert=True)
    except Exception as e:
        print(item)
        raise e
